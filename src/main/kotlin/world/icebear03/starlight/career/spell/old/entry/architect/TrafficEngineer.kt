...
import world.icebear03.starlight.career.spell.old.setFinish
...
object TrafficEngineerActive {
    ...
    fun initialize() {
        "追加动力".setDischarge { id, level ->
            ...
            "§a技能 ${display(id)} §7释放成功，载具在§a5秒§7内速度加快§e${percent}%"
        }
        "追加动力".setFinish { _, level ->
            val percent = 20 + 10 * level
        ...
        "备用载具".setDischarge skill@{ id, _ ->
            if (this.hasBlockAside(Material.WATER, 1) ||
                this.hasBlockAside(TrafficEngineerSet.ICE_BLOCKS.types.also { listOf(Material.ICE, Material.FROSTED_ICE) }, 1)
            ) {
                this.world.spawnEntity(this.location.add(0.0, 1.0, 0.0), EntityType.BOAT)
                return@skill "§a技能 ${display(id)} §7释放成功，船只已经召唤"
            }
            if (this.hasBlockAside(TrafficEngineerSet.RAILS.types, 1)) {
                this.world.spawnEntity(this.location, EntityType.MINECART)
                return@skill "§a技能 ${display(id)} §7释放成功，矿车已经召唤"
            }
            "§a技能 ${display(id)} §7释放成功，但是没有载具匹配当前位置"
        }
        "游弋信风".setDischarge { id, _ ->
            ...
            "§d顿悟 ${display(id)} §7释放成功，获得效果 §b海豚的恩惠"
        }
    }
}
...
object TrafficEngineerPassive {
    ...
    fun event(event: BlockPlaceEvent) {
        ...
        val level = player.career().getSpellLevel("路线规划")
