    ...
    fun initialize() {
        ...
        "牧原欢歌".discharge { name, level ->
            ...
            if (type == Material.SHEARS) {
                getNearbyEntities(range, range, range).filterIsInstance<Sheep>().forEach { goat ->
                    shear(PlayerShearEntityEvent(this, goat, item, EquipmentSlot.HAND))
                }
    ...
    fun drink(event: PlayerItemConsumeEvent) {
        if (event.item.type == Material.MILK_BUCKET) {
            ...
            if (biome.toString().contains("PLAIN") && player.meetRequirement("游牧习俗")) {
                player.effect(PotionEffectType.HEAL, 1, 1)
