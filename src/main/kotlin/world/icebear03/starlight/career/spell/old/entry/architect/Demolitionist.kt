package world.icebear03.starlight.career.spell.old.entry.architect
...
import world.icebear03.starlight.career.spell.old.passive.limit.LimitType
import world.icebear03.starlight.career.spell.old.passive.recipe.CraftHandler
import world.icebear03.starlight.career.spell.old.passive.recipe.CraftResult
import world.icebear03.starlight.career.spell.old.passive.recipe.addSpecialRecipe
import world.icebear03.starlight.career.spell.old.passive.recipe.registerShapedRecipe
import world.icebear03.starlight.career.spell.old.setDischarge
import java.util.*

object DemolitionistActive {

    fun initialize() {
        "气浪行者".setDischarge { id, level ->
            val duration = 2 + level
            val percent = 35 + 15 * level
            "§a技能 ${display(id)} §7释放成功，接下来§e${duration}秒§7内受到爆炸伤害减少§e${percent}%"
        }
        "手摇TNT火炮".setDischarge { id, level ->
            "§d顿悟 ${display(id)} §7释放成功，接下来§e30秒§7内箭矢变为TNT"
        }
        "前沿爆破线".setDischarge { id, level ->
            val amount = 2 + level
            ...
            "§a技能 ${display(id)} §7释放成功，发射出§e${amount}个§7点燃的TNT"
        }
    }
    ...
    fun damaged(event: EntityDamageEvent) {
        ...
        if (entity.isDischarging("气浪行者")) {
            val percent = 35 + 15 * entity.career().getSpellLevel("气浪行者")
...
object DemolitionistPassive {
    ...
    fun initialize() {
        ...
        CraftHandler.registerHigh(DemolitionistSet.TNT.types) skill@{ player, type ->
            if (player.meetRequirement("精炼炸药")) {
                if (Math.random() <= 0.2) {
        ...
        CraftHandler.registerLowest(DemolitionistSet.TNT.types) skill@{ player, _ ->
            val failPercent = when (player.career().getSpellLevel("稳定三硝基甲苯")) {
                1 -> 0.25
                2 -> 0.1
                3 -> 0.0
                else -> 0.5
            ...
            if (Math.random() <= failPercent) {
                if (Math.random() <= failPercent) {
                    ...
                    CraftResult.FAIL to "§a生涯系统 §7>> 炸药合成时爆炸了，升级技能 ${display("稳定三硝基甲苯")} §7以规避"
                } else {
                    CraftResult.FAIL to "§a生涯系统 §7>> 炸药合成失败，升级技能 ${display("稳定三硝基甲苯")} §7以提升成功率"
                }
            } else {
    ...
    fun igniteBlock(event: BlockIgniteEvent) {
        ...
        if (!player.career().hasBranch("爆破师")) {
            if (Math.random() >= 0.2)
                submit {
    ...
    fun damagedByExplosion(event: EntityDamageEvent) {
        ...
        if (entity.career().hasBranch("爆破师")) {
