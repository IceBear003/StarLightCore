...
import world.icebear03.starlight.career.spell.old.passive.recipe.registerShapelessRecipe
...
object StructuralEngineerActive {
    fun initialize() {
        "识色敏锐".setDischarge { id, _ ->
            "§a技能 ${display(id)} §7释放成功，下一次合成染色方块时会获得额外同种方块"
        }

        "凌空创想".setDischarge { id, skillLevel ->
            val player = this
            ...
            "§a技能 ${display(id)} §7释放成功，即刻获得§f速度I§7，可利用脚手架获得更多增益"
        }
...
object StructuralEngineerPassive {
    ...
    fun breakBlock(event: BlockBreakEvent) {
        ...
        if (StructuralEngineerSet.DYED_BLOCK.types.contains(type)) {
            if (player.meetRequirement("奇珍颜料")) {
                if (Math.random() <= 0.2) {
